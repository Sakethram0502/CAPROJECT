...
$uploadMap = [];
$uRes = $conn->query(
    "SELECT id, reg_no, semester, document_name, ppt_name, code_name, certificate_name
...
foreach ($submissions as &$s) {
    ...
    if (isset($uploadMap[$reg])) {
        ...
        $s['certificate_name'] = $uploadMap[$reg]['certificate_name'] ?? null;
    } else {
        $s['upload_id']        = null;
        $s['document_name']    = null;
        $s['ppt_name']         = null;
        $s['code_name']        = null;
        $s['certificate_name'] = null;
    }
}
unset($s);
...
<head>
    ...
    <style>
        ...
        /* File modal buttons – add .fm-cert */
        .fm-btn { display: block; width: 100%; padding: 10px 14px; border-radius: 8px; font-size: 0.88em; text-decoration: none; margin-bottom: 8px; border: 1px solid; transition: opacity 0.16s; font-family: 'DM Sans', sans-serif; font-weight: 500; }
        .fm-btn:hover { opacity: 0.78; }
        .fm-doc  { background: var(--green-bg);   color: var(--green-txt); border-color: var(--green-bd); }
        .fm-ppt  { background: var(--navy-faint); color: var(--navy);      border-color: var(--border); }
        .fm-code { background: #f0f9ff;            color: #0369a1;          border-color: #bae6fd; }
        .fm-zip  { background: #f5f0ff;            color: #5b21b6;          border-color: #c4b5fd; margin-top: 4px; }
        .fm-cert { background: #f0f9ff;            color: #0369a1;          border-color: #bae6fd; }
    </style>
</head>
<body>
...
<div class="dashboard-wrapper">
    ...
    <div class="dashboard-layout">
        ...
        <main class="dashboard-main">
            ...
            <div class="tab-bar">
                <button class="tab-btn <?php echo $active_tab==='all'?'active':''; ?>" onclick="switchTab('all')">All Students</button>
                <button class="tab-btn <?php echo $active_tab==='no_upload'?'active':''; ?>" onclick="switchTab('no_upload')">
                    No Upload <?php if(count($no_upload)>0): ?><span class="tab-count"><?php echo count($no_upload); ?></span><?php endif; ?>
                </button>
                <button class="tab-btn <?php echo $active_tab==='no_review'?'active':''; ?>" onclick="switchTab('no_review')">
                    No Review <?php if(count($no_review)>0): ?><span class="tab-count"><?php echo count($no_review); ?></span><?php endif; ?>
                </button>
            </div>

            <?php if($view === 'approvals'): ?>
            <h2 class="section-heading" style="margin-bottom:2px;">Student Update Requests</h2>
            <p class="sub-heading"><?php echo count($pendingRequests); ?> pending</p>
            ...
            <div style="background:#fff;border:1px solid var(--border);border-left:4px solid #f59e0b;border-radius:12px;padding:18px 20px;margin-bottom:14px;box-shadow:var(--shadow-sm);">
                <div style="display:flex;align-items:flex-start;justify-content:space-between;flex-wrap:wrap;gap:10px;margin-bottom:14px;">
                    <div>
                        ...
                        <div style="font-size:0.78rem;color:var(--text-muted);margin-top:2px;"><?php echo htmlspecialchars($req['reg_no']); ?> · <?php echo date('d M Y, h:i A', strtotime($req['requested_at'])); ?> · <strong>Sem: <?php echo htmlspecialchars($req['semester_key'] ?? '—'); ?></strong></div>
                    </div>
                ...
                <div style="background:#fafbfc;border:1px solid var(--border-light);border-radius:8px;padding:10px 14px;margin-bottom:14px;">
                    <div style="font-size:0.72rem;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;color:var(--text-faint);margin-bottom:4px;">Reason</div>
                ...
                <form method="POST" style="display:flex;flex-direction:column;gap:10px;">
                    ...
                    <div class="form-group" style="margin:0;">
                        <label style="font-size:0.82rem;font-weight:600;color:var(--text-body);margin-bottom:5px;display:block;">Remark (optional)</label>
                        <input type="text" name="guide_remark" placeholder="e.g. Approved, update carefully / Rejected"
                               style="width:100%;padding:8px 12px;border-radius:7px;border:1.5px solid var(--border);font-size:0.88rem;outline:none;">
                    </div>
                    <div style="display:flex;gap:10px;">
                        <button type="submit" name="req_action" value="approved"
                                style="flex:1;padding:9px;border-radius:8px;border:none;background:#16a34a;color:#fff;font-weight:700;font-size:0.88rem;cursor:pointer;">Approve</button>
                        <button type="submit" name="req_action" value="rejected"
                                style="flex:1;padding:9px;border-radius:8px;border:1px solid #fca5a5;background:#fff;color:#dc2626;font-weight:700;font-size:0.88rem;cursor:pointer;">Reject</button>
                    </div>
                </form>
            </div>
            <?php endforeach; ?>
                ...
                <div class="student-card <?php echo empty($row['upload_id']) ? 'no-upload' : ''; ?>">
                    ...
                    <div class="reviews-row">
                        ...
                        <div class="action-cell">
                            ...
                            <?php if(!empty($row['upload_id'])): ?>
                            <button class="view-btn" onclick="openFilesModal(
                                ...
                                <?php echo !empty($row['certificate_name'])?1:0; ?>
                            )">📁 View Files</button>
                            <?php else: ?>
            ...
            <div id="tab-no_upload" class="tab-section" style="display:<?php echo $active_tab==='no_upload'?'block':'none'; ?>;">
                ...
                <div class="pending-export-bar">
                    <p>⚠️ <?php echo count($no_upload); ?> students have not uploaded documents.</p>
                    <div class="pending-export-btns">
                        <a href="pdf_generator.php?course=<?php echo $rp; ?>&format=pdf&pending=upload" class="exp-btn pdf">📄 Export PDF</a>
                        <a href="pdf_generator.php?course=<?php echo $rp; ?>&format=excel&pending=upload" class="exp-btn excel">📊 Export Excel</a>
                    </div>
                </div>
                ...
                <?php endforeach; ?>
                <?php else: ?>
                <div style="text-align:center;padding:48px;color:var(--text-muted);">✅ All students have uploaded documents.</div>
                <?php endif; ?>
            </div>
            ...
            <div id="tab-no_review" class="tab-section" style="display:<?php echo $active_tab==='no_review'?'block':'none'; ?>;">
                ...
                <div class="pending-export-bar">
                    <p>⚠️ <?php echo count($no_review); ?> students have not received any marks.</p>
                    <div class="pending-export-btns">
                        <a href="pdf_generator.php?course=<?php echo $rp; ?>&format=pdf&pending=review" class="exp-btn pdf">📄 Export PDF</a>
                        <a href="pdf_generator.php?course=<?php echo $rp; ?>&format=excel&pending=review" class="exp-btn excel">📊 Export Excel</a>
                    </div>
                </div>
                ...
                <?php endforeach; ?>
                <?php else: ?>
                <div style="text-align:center;padding:48px;color:var(--text-muted);">✅ All students have received at least one review.</div>
                <?php endif; ?>
            </div>
    ...
    <div class="modal-card floating">
        ...
        <form method="POST" action="staff_dashboard.php?view=<?php echo $view; ?>" class="form-glass modal-form">
            ...
            <div class="form-group">
                <p style="margin:0;font-size:0.9em;color:var(--text-muted);">Student: <span id="dispName" style="color:var(--text-dark);font-weight:600;"></span></p>
            ...
            <div class="modal-actions">
                <button type="button" class="modal-close" style="padding:9px 20px;border-radius:7px;border:2px solid var(--border-strong);background:#f1f5f9;color:#334155;font-weight:600;font-size:0.9rem;cursor:pointer;">Cancel</button>
...
<div class="modal-overlay" id="filesModal">
    <div class="modal-card floating" style="max-width:400px;">
        ...
        <div style="margin-top:16px;text-align:right;">
            <button type="button" id="filesModalClose" style="background:none;border:1px solid var(--border);border-radius:100px;color:var(--text-muted);cursor:pointer;font-size:0.85em;padding:6px 16px;">Close</button>
        </div>
    </div>
</div>
...
// Files modal – now supports hasCert
const filesModal = document.getElementById('filesModal');

function openFilesModal(id, reg, name, hasDoc, hasPpt, hasCode, hasCert) {
    ...
    if (parseInt(hasCert)) {
        const a = document.createElement('a');
        a.href = 'download_file.php?id=' + id + '&type=certificate';
        a.className = 'fm-btn fm-cert';
        a.innerHTML = '🏅 &nbsp;Download Certificate';
        container.appendChild(a);
    }
...
document.getElementById('filesModalClose').addEventListener('click', () => filesModal.classList.remove('open'));
window.addEventListener('click', e => {
